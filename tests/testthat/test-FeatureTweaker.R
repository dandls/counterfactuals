...
test_that("Returns correct output format for numeric columns only", {
  ...
  x_interest = X[10L, ]
  n = 2L
  pred = Predictor$new(rf, data = iris, y = "Species", class = "versicolor")
...
test_that("Returns error message if factors are in training data", {
  # featureTweakR package cannot deal with factors in training data
  set.seed(54542142)
  ...
  pred = Predictor$new(rf, data = test_df, y = "am", type = "class")
  n = 2L
...
test_that("Init works for classification tasks only", {
  ...
  true_y = mtcars$mpg
  rf_regr = randomForest::randomForest(X, true_y, ntree = 20L)
  x_interest = X[10L, ]
  n = 2L
  pred_regr = Predictor$new(rf_regr, data = mtcars, y = "mpg")
...
test_that("If `desired_outcome` is specified for binary class, it is set to the opposite of `x_interest`", {
  ...
  true_y = iris[, ncol(iris)]
  rf = randomForest::randomForest(X, true_y, ntree = 20L)
  iris_pred = iml::Predictor$new(rf, data = iris, y = "Species", type = "class", class = "versicolor")
  n = 2L
...
test_that("Can handle non-numeric target classes", {
  ...
  X = test_data[, c("a", "b")]
  true_y = test_data$cl
  rf_pima = randomForest::randomForest(X, true_y, ntree = 20L)
