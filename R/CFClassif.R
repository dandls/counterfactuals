#' CF (Multi-Objective Counterfactual Fairness) for Classification Tasks
#' 
#' @template moc_info
#' 
#' @examples 
...
#' 
#' @export
CFClassif = R6::R6Class("CFClassif", inherit = CounterfactualMethodClassif,

  public = list(
    #' @description Create a new `CFClassif` object.
    #' @template predictor
    ...
    initialize = function(predictor, protected = NULL, epsilon = NULL, fixed_features = NULL, max_changed = NULL, mu = 20L, 
      ...
      if (use_conditional_mutator) {
        ...
        if (!requireNamespace("mlr3pipelines", quietly = TRUE)) {
          stop("Package 'mlr3pipelines' needed for this function to work. Please install it.", call. = FALSE)
        }
        if (!requireNamespace("mlr3learners", quietly = TRUE)) {
          stop("Package 'mlr3learners' needed for this function to work. Please install it.", call. = FALSE)
        }
    ...
    find_counterfactuals = function(x_interest, desired_class = NULL, desired_prob = c(0.5, 1)) {
      ...
      assert_choice(desired_class, choices = levels(predictor$data$X[[private$protected]]))
  ...
  private = list(
    ...
    get_prot_predictor = function() {
        data = private$predictor$data$X
        fit_prot_predictor(data, private$protected)
    },
