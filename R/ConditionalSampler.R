ConditionalSampler = R6Class(
  ...
  private = list(
    ...
    fit_conditional = function() {
      # TODO: Check if the following packages are installed in the MOCClassif and MOCRegr
      # "trtf", "partykit", "variables", "basefun"
      y_unique = unique(private$y)
      if (is.numeric(y_unique) && length(y_unique[!is.na(y_unique)]) > 2L) {
    ...
    create_y_grid = function() {
      ...
      if (is.character(y) || is.factor(y)) {
        ...
        y_grid = seq.int(min(y), max(y), length.out = len)
      } else {
