...
To illustrate the `counterfactuals` workflow for classification tasks, we search for counterfactual explanations for diabetes tested patients with `MOC` (Dandl et al. 2020). 
...
For `x_interest` the model predicts a class probability of 63.4% for a diabetes disease.
...
in `x_interest` and negative values indicate a decrease. For factors, the counterfactual feature value is displayed if it 
...
counterfactuals explanations for housing prices with `WhatIf` (Wexler et al. 2019).
