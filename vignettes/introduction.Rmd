...
To demonstrate the `counterfactuals` workflow for classification tasks, we search for counterfactual explanations for diabetes tested patients with `MOC`. 
...
The method `$plot_search()` visualizes all individuals of all generations in a scatter plot with two objectives on the axes.

```{r}
moc_classif$plot_search()
```

It is also possible to plot the evolution of the mean and minimum objective values together with the dominated hypervolume over the generations
...
First we train a model to predict `medv`. <br> 
...
regression tasks we only need to specify `desired_outcome`, unlike `desired_class` and `desired_probs` for
