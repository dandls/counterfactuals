...
```{r}
set.seed(20210816)
rf = randomForest::randomForest(diabetes ~ ., data = PimaIndiansDiabetes[-499L, ])
```
...
For `x_interest`, the model predicts a diabetes probability of `r round(predictor$predict(x_interest), 2)[2]`.
...
With the `fixed_features` argument, we can fix non-actionable features, here `pregnant` and `age`.
...
if (!file.exists("introduction-res/cfactuals.RDS")) {
  moc_classif = MOCClassif$new(predictor, epsilon = 0, fixed_features = c("pregnant", "age"), quiet = TRUE)
  cfactuals = moc_classif$find_counterfactuals(x_interest, desired_class = "pos", desired_prob = c(0, 0.4))
  dir.create("introduction-res")
  saveRDS(moc_classif, file = "introduction-res/moc_classif.RDS")
  saveRDS(cfactuals, file = "introduction-res/cfactuals.RDS")
}
...
cfactuals$plot_parallel(feature_names = NULL, row_ids = NULL, 
 digits_min_max = 2L)
```
...
```{r, fig.height=3}
cfactuals$plot_surface(feature_names = c("glucose", "insulin"))
```
...
`NICE` (Brughmans et al. (2022)) to search for counterfactuals for housing prices. 
...
This time we use a support vector machine trained with the `mlr3` package. 

```{r}
tsk = mlr3::TaskRegr$new(id = "boston", backend = BostonHousing[-1L,], target = "medv")
tree = lrn("regr.rpart")
...
For `x_interest`, the model predicts a median housing value of `r round(predictor$predict(x_interest), 2)[1]`.
...
`margin_correct = 0.5`. In this example, we aim for plausible counterfactuals 
in additional to sparse ones, such that we set `optimization = "plausiblity"`. 
...
Explanations.” Technical report, arXiv 2104.07411 v2. URL https://arxiv.org/abs/
2104.07411.
