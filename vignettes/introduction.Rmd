...
To demonstrate the workflow in classification settings, we search for counterfactual explanations for diabetes tested
patients with `MOC`. 
...
only contain `integer` values for them. (e.g. no `2.76` pregnancies).
...
```{r}
rf = randomForest(diabetes ~ ., data = PimaIndiansDiabetes)
```
...
```{r}
predictor = Predictor$new(rf, data = PimaIndiansDiabetes, y = "diabetes", type = "prob")
```


### Find counterfactuals

Here we simply use an observation from our training data as `x_interest`.

```{r}
x_interest = PimaIndiansDiabetes[500L, ]
predictor$predict(x_interest)
```

Now we can examine which risk factors need to be changed to obtain a predicted probability of
at least 70% for a diabetes disease.
...
```{r, eval=FALSE}
cfactuals = moc_classif$find_counterfactuals(x_interest, desired_class = "pos", desired_prob = c(0.7, 1))
```
...
We can use the `$predict()` method to predict the target values of the counterfactuals found.
...
`nr_changed` and `dist_train`). Setting the `show_diff` argument to `TRUE` shows the counterfactuals as their difference
...
The white dot in the surface plot represents `x_interest`. All counterfactuals that only differ from `x_interest` 
in the selected features are displayed as black dots.

```{r, fig.height=3.5}
cfactuals$plot_surface(feature_names = c("mass", "pregnant"))
```
...
`$plot_search()` plots two objective values of all individuals over the generations.

```{r}
moc_classif$plot_search()
```

To plot the evolution of the mean and minimum objective values together with the dominated hypervolume, we can use the 
...
```{r}
predictor = Predictor$new(rf, data = BostonHousing, y = "medv")
```
...
```{r}
x_interest = BostonHousing[1L, -ncol(BostonHousing)]
predictor$predict(x_interest)
```

As we want to use `WhatIf` for a regression model, we initialize a `WhatIfRegr` object.
...
```{r}
cfactuals = whatif_regr$find_counterfactuals(x_interest, desired_outcome = c(28, 30))
```
